...
public class EquipWeapon {
    ...
    private int amountOfAttackIncreased = 5;
    @Given("the inventory item is available and it's a unequipped weapon")
    public void theInventoryItemIsAvailableAndItSAUnequippedWeapon() {
    ...
    public void thePlayerEquipsTheWeapon() {
        try {
            HttpHeaders headers = new HttpHeaders();
            headers.set("Content-Type", "application/json");

            response = new RestTemplate().exchange(BASE_URL + port + "/api/inventoryItems/" + inventoryItem.getId() + "/equipWeapon", HttpMethod.PUT, null, String.class);
    ...
    public void thePlayerSAttributesShouldBeUpdatedWithTheWeaponAttributes() {
    ...
    public void theWeaponIsMarkedAsEquipped() {
